...
public class SinglyLinkedList<E>{
    ...
    public Node<E> addAfter(Node<E> p, E x){
        if(isEmpty()){
    ...
    public void deleteAfter(Node<E> p){
        if(isEmpty() || size == 1){
            // to use this method, list size must be > 1
        }else if(p == null){
    ...
    public Node<E> addBefore(Node<E> p, E x){
        if(isEmpty()){
    ...
    private Node<E> merge(Node<E> A, Node<E> B){
        if(A == null) {this.tail = B;return B;}
        if(B == null) {this.tail = A;return A;}
